...
V_i = 1000; %V_i represents Reactor volume (in liters)
...
for i = 2:N
    ...
    %The function getRatesofChange is used to calculate the respective
    %values of rate of change in concentration of all the reactants and
    %products
    ...
    % To ensure concentrations are non-negative [This will ensure that the
    % concentration remains non-negative .If found negative it will make
    % the concentration equal to zero.]
...
figure(1);
plot(Time,C_glucose,'r','LineWidth',1.5);
hold on;
plot(Time,C_yeast,'g','LineWidth',1.5);
plot(Time,C_ethanol,'b','LineWidth',1.5);
xlabel('Time (hours)');
...
%each rate of change is calculated using monod equation .The equation
%doesn't comprise of the Volume of the Reactor as we have assumed the
%volume to be fairly constant in whole process.]
